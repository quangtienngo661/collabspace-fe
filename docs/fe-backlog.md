...
> **Cập nhật:** 2026-06-18  
> **Đối chiếu codebase:** 2026-06-18 — MVP API integration Done; Tiến workspace polish **Done** (A1–A9, D2 WS); Tín task/dashboard/notification polish **Done** (B2, B3, B6–B9, D4, F3)  
...
**Đã xử lý:** Cột "Joined" (`hidden lg:table-cell`) dùng `timeAgo(member.joinedAt)` + tooltip `toLocaleString()`; pending rows hiện "Invited {timeAgo}".
...
**Đã xử lý:** Badge "Pending" amber, row opacity/style riêng, status "Invited", hiện `expiresAt` khi có.
...
**Đã xử lý:** Dropdown tách "Demote to member" (owner → manager) và "Remove from workspace" / "Leave workspace" (confirm dialog).
...
**Đã xử lý:** Dưới `EmptyState`, link `/invitations` với copy "Waiting for an invite? Check your invitations."
...
**Đã xử lý:** `ProjectListPage.tsx` chỉ render menu `⋯` khi `canManageProjects`.
...
**Đã xử lý:** Skeleton pulse trên stats khi `!enriched`; hiện số thật sau khi `enrichWorkspacesStats` xong.
...
**Đã xử lý:** Header `{members.length} Members` + suffix `· {invitations.length} pending` khi có pending.
...
**Đã xử lý:** `formatDate(p.createdAt)` (locale short date) + tooltip `toLocaleString()` trên tab Projects.
...
### B2 · DashboardPage — Empty state khi chưa có workspace — ✅ Done · **Tín**

**Vấn đề:** Khi user chưa có workspace nào, Dashboard chỉ hiện 1 alert card mà không dẫn hướng rõ.

**Đã xử lý:** Dashboard empty state có copy rõ hơn, action "Create workspace", "Browse Workspaces", "Check Invitations", và block pending invitations khi có notification invite.

---

### B3 · DashboardPage — Stat cards thiếu context — ✅ Done · **Tín**

**Vấn đề:** 6 stat cards chỉ hiện số, không có tooltip hay sparkline. "15 tasks" — của workspace nào? Khoảng thời gian nào?

**Đã xử lý:** Mỗi KPI card hiển thị context nhỏ theo workspace hiện tại; notification card ghi rõ phạm vi toàn tài khoản.
...
### B6 · TaskDetailSheet — Assignee không có avatar đầy đủ — ✅ Done · **Tín**
...
**Đã xử lý:** `TaskDetailSheet` dùng `useWorkspaceMemberUsers` để hydrate assignee/creator profile, hiển thị avatar đầy đủ khi `avatarUrl` có sẵn, fallback initials khi thiếu.

---

### B7 · NotificationsPage — Type label fallback — ✅ Done · **Tín**
...
**Đã xử lý:** Bổ sung label/icon và filter aliases cho:
```ts
...
### B8 · NotificationsPage — Unread badge và grouping — ✅ Done · **Tín**
...
**Đã xử lý:** Unread rows có left border + background nhẹ, read rows trở về nền thường, Inbox có header "Mark all as read", và actions reload cả page state lẫn `NotificationsContext`.

---

### B9 · DashboardPage — KPI & chart sai khi >50 tasks — ✅ Done · **Tín**
...
**Đã xử lý:** `DashboardPage` dùng `taskApi.getBoard({ workspaceId })` cho KPI, chart, và "My Assigned Tasks", tránh default limit 50 của `GET /tasks`.
...
### D2 · Mobile — bảng members/tasks bị cắt — ✅ Done · **Tiến** + **Thọ**

**Vấn đề:** Các `<Table>` trong `WorkspaceDetailPage` và `UsersDirectoryPage` không có horizontal scroll trên mobile.

**Đã xử lý:**
- **Tiến** — `WorkspaceDetailPage.tsx`: bọc bảng members trong `<div className="overflow-x-auto">`
- **Thọ** — `UsersDirectoryPage.tsx`: bọc `<Table>` tương tự
...
### D4 · TaskDetailSheet — Không có link "Open in full page" — ✅ Done · **Tín**

**Vấn đề:** Task detail chỉ mở trong sheet (slide-in). Không có cách share URL của task trực tiếp (dù `?task=id` đã có).

**Đã xử lý:** Thêm icon copy link ở header `TaskDetailSheet`, sinh URL `/workspaces/:workspaceId/projects/:projectId?task=:taskId` khi có project và fallback `/workspaces/:workspaceId/projects?task=:taskId`.
...
### Ngô Quang Tiến · Workspace — ✅ Done (FE polish)
...
P3
[x] A4 · "Remove from workspace" tách khỏi demote
[x] A5 · Workspace empty state — link invitations
[x] D2 · Mobile table scroll — WorkspaceDetailPage
```

> **Còn lại (không thuộc backlog A):** invite validation rules admin/member trùng → **BE** (`INVITE_*`); FE pre-check + `formatInviteError` đã có.

### Võ Trung Tín · Task & Notification — ✅ Done

```
P0
[x] B9 · Dashboard KPI + chart — không bị cắt 50 tasks
[x] B2 · Dashboard empty state — copy/link invitations

P1
[x] B7 · Notification type labels đầy đủ
[x] B8 · Unread visual trong NotificationsPage

P2
[x] B3 · Stat cards có workspace context
[x] B6 · Assignee avatar đầy đủ trong TaskDetailSheet

P3
[x] D4 · TaskDetailSheet copy link ?task=
```
...
[x] D2 · Mobile table scroll — UsersDirectoryPage
```
