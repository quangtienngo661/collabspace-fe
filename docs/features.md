...
| Workspace & project | **Partial** | **Tiến** | Nhiều polish **FE-only**; invite rules **Needs BE** |
...
| Platform admin | **Partial** | **Thọ** | CRUD **Done**; overview KPI **FE-only** (C9) |
| Presence & directory | **Done** | **Thọ** | Done |
| Shell UX | **Partial** | **Thọ** *(D1, D2, C7)* | Breadcrumb **Done** |
...
|-----------|-------|-----|------------|------------------|
| Danh sách workspace | `/workspaces` | `GET /workspaces` | **Partial** | **Tiến** — A7: không flash `0 members/projects` |
| Tạo / xóa workspace | `/workspaces` | `POST/DELETE /workspaces` | **Done** | — |
| Chi tiết workspace | `/workspaces/:id` | `GET/PATCH /workspaces/:id` | **Partial** | **Tiến** — A8, A2, A3 |
| Members tab | tab `members` | `GET .../members`, `DELETE`, `PATCH` role | **Done** | A4 label dropdown (P3) |
| Mời thành viên | dialog Invite | `POST .../invite` | **Partial** | **Tiến** UI dialog · **Thọ** map lỗi ✅ · **BE** validation |
| Projects CRUD | `.../projects` | project endpoints | **Done** | **Tiến** — A6 ẩn ⋯, A9 format `createdAt` |
...
**Tiến (fe-backlog A):** A2 `joinedAt`, A3 pending badge, A6 project menu, A8 member count, A9 date format, A5 empty state link invitations (P3).
...
| `commentCount` trên card | Kanban | board/list API | **Done** | — |
...
| Workspaces | Workspaces | `/workspaces/admin/all` | **Partial** | C10 owner name, C1 info modal (P3) |
| Broadcast | Broadcast | `POST .../broadcast` | **Done** | — |
| Health probe | mount | `/admin/health` | **Done** | — |
...
| Mobile table scroll (D2) | thiếu | **Thọ** + **Tiến** (WS tables) | FE-only |
...
|---------|-----|------|------|
| **P0** | A7 | Không flash `0 members/projects` khi enrich | `WorkspaceListPage.tsx`, `WorkspacesContext.tsx` |
| **P0** | A8 | Header đếm members ≠ pending invitations | `WorkspaceDetailPage.tsx` |
| P1 | A6 | Ẩn ⋯ project card khi `!canManageProjects` | `ProjectListPage.tsx` |
| P2 | A2 | Cột Joined — `timeAgo(joinedAt)` | `WorkspaceDetailPage.tsx` |
| P2 | A3 | Pending invitation badge / style riêng | `WorkspaceDetailPage.tsx` |
| P2 | A9 | Project `createdAt` không raw ISO | `ProjectListPage.tsx` |
| P3 | A4, A5 | Label remove rõ nghĩa; empty state → invitations | `WorkspaceDetailPage.tsx` |
| P3 | D2 | `overflow-x-auto` bảng members | `WorkspaceDetailPage.tsx` |
