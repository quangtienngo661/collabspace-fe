...
| Task & board | **Done** | **Tín** | Kanban, dashboard KPI, task sheet polish Done |
...
| Dashboard KPI + chart | `/dashboard` | `GET /tasks/board` | **Done** | B9 uses board data, not default 50-item list ✅ |
...
**Tín (fe-backlog B):** ✅ Done — B2 dashboard empty state, B3 stat cards context, B6 assignee avatar, B7 notification labels *(shared với Notifications)*, B8 unread highlight, D4 task link.
...
|---------|-----|------|------|
| **P0** | B9 | Dashboard KPI/chart — dùng `getBoard`, không cắt 50 task ✅ | `DashboardPage.tsx` |
